  ...
  return (
    <main className="heyy-page heyy-page-grid py-8 sm:py-10">
      <PageContainer>
        ...
        {loading ? <LoadingTable/> : error ? <ErrorState message={error} retry={()=>void loadLibrary()}/> : filtered.length === 0 ? <EmptyLibrary/> : (
          <AssetTable
            items={filtered}
            onOpen={(item)=>{setActionError("");setActive(item);}}
...
function LoadingTable(){return <section className="mt-5 overflow-hidden rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-strong)]"><div className="hidden grid-cols-[minmax(240px,1.6fr)_minmax(160px,1fr)_120px_minmax(150px,.9fr)_110px_110px_96px] gap-4 border-b border-[var(--border)] bg-[var(--surface-hover)] px-5 py-3 text-[.6rem] font-black uppercase tracking-[.13em] text-[var(--text-muted)] xl:grid"><span>Asset</span><span>Project</span><span>Studio</span><span>Type</span><span>Status</span><span>Updated</span><span className="text-right">Actions</span></div>{Array.from({length:8}).map((_,index)=><div key={index} className="grid gap-3 border-b border-[var(--border)] px-4 py-4 last:border-b-0 sm:grid-cols-[64px_1fr] xl:grid-cols-[minmax(240px,1.6fr)_minmax(160px,1fr)_120px_minmax(150px,.9fr)_110px_110px_96px] xl:items-center xl:gap-4 xl:px-5"><div className="h-12 w-16 animate-pulse rounded-xl bg-[var(--surface-hover)] xl:hidden"/><div className="space-y-2"><div className="h-3.5 w-2/3 animate-pulse rounded bg-[var(--surface-hover)]"/><div className="h-3 w-1/3 animate-pulse rounded bg-[var(--surface-hover)]"/></div>{Array.from({length:5}).map((__,cell)=><div key={cell} className="hidden h-3 w-3/4 animate-pulse rounded bg-[var(--surface-hover)] xl:block"/>)}<div className="hidden h-8 w-20 animate-pulse rounded-full bg-[var(--surface-hover)] xl:block"/></div>)}</section>;}

function AssetTable({items,onOpen,onDownload}:{items:LibraryItem[];onOpen:(item:LibraryItem)=>void;onDownload:(item:LibraryItem)=>void}){return <section className="mt-5 overflow-hidden rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-strong)] shadow-[var(--shadow-card)]"><div className="hidden grid-cols-[minmax(240px,1.6fr)_minmax(160px,1fr)_120px_minmax(150px,.9fr)_110px_110px_96px] gap-4 border-b border-[var(--border)] bg-[var(--surface-hover)] px-5 py-3 text-[.6rem] font-black uppercase tracking-[.13em] text-[var(--text-muted)] xl:grid"><span>Asset</span><span>Project</span><span>Studio</span><span>Type</span><span>Status</span><span>Updated</span><span className="text-right">Actions</span></div><div className="divide-y divide-[var(--border)]">{items.map((item)=>{const kind=previewKind(item);return <article key={item.sourceKey} className="grid gap-3 px-4 py-4 transition hover:bg-[var(--surface-hover)] sm:grid-cols-[72px_1fr_auto] sm:items-center xl:grid-cols-[minmax(240px,1.6fr)_minmax(160px,1fr)_120px_minmax(150px,.9fr)_110px_110px_96px] xl:gap-4 xl:px-5"><button type="button" onClick={()=>onOpen(item)} className="flex min-w-0 items-center gap-3 text-left sm:col-span-2 xl:col-span-1"><span className="grid h-14 w-18 shrink-0 place-items-center overflow-hidden rounded-xl bg-[linear-gradient(135deg,var(--surface-hover),var(--accent-soft))]">{item.previewUrl&&kind==="image"?<img src={item.previewUrl} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover"/>:kind==="pdf"?<File size={20} className="text-[var(--text-muted)]"/>:<FileImage size={20} className="text-[var(--text-muted)]"/>}</span><span className="min-w-0"><span className="block truncate text-sm font-black">{item.title}</span><span className="mt-1 block truncate text-[.64rem] font-bold text-[var(--text-muted)]">V{item.version}{item.productionReady?" · Production ready":""}</span></span></button><div className="min-w-0 text-xs font-bold text-[var(--text-secondary)]"><span className="xl:hidden text-[.58rem] font-black uppercase tracking-[.12em] text-[var(--text-muted)]">Project · </span><span className="truncate">{item.projectName}</span></div><div className="text-xs font-black"><span className="xl:hidden text-[.58rem] uppercase tracking-[.12em] text-[var(--text-muted)]">Studio · </span>{STUDIO_LABELS[item.studio]}</div><div className="min-w-0 truncate text-xs font-bold text-[var(--text-secondary)]"><span className="xl:hidden text-[.58rem] font-black uppercase tracking-[.12em] text-[var(--text-muted)]">Type · </span>{item.assetTypeLabel}</div><div><StatusPill tone={statusTone(item.status)}>{item.status}</StatusPill></div><div className="text-xs font-bold text-[var(--text-muted)]">{formatDate(item.updatedAt||item.createdAt)}</div><div className="flex items-center justify-end gap-1"><button type="button" onClick={()=>onOpen(item)} className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--border)] text-[var(--text-secondary)] transition hover:border-[var(--accent-border)] hover:text-[var(--accent-strong)]" aria-label={`Open ${item.title}`} title="Open asset"><ExternalLink size={14}/></button><button type="button" onClick={()=>onDownload(item)} className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--border)] text-[var(--accent-strong)] transition hover:border-[var(--accent-border)] hover:bg-[var(--accent-soft)]" aria-label={`Download ${item.title}`} title="Download"><Download size={14}/></button></div></article>;})}</div></section>;}
