...
type Version = {
  ...
  status:Status; creditCost:number|null; changeSummary:string|null; userNote:string|null;
...
async function readJsonResponse(response:Response):Promise<any>{const text=await response.text();if(!text)return {};try{return JSON.parse(text);}catch{return{error:response.ok?"The server returned an invalid response.":`Version history request failed (${response.status}).`};}}
...
export default function VersionHistory(){
  ...
  const filtered=useMemo(()=>families.filter(f=>{if(!showSingleVersions&&f.versions.length<2)return false;if(studio!=="all"&&f.studio!==studio)return false;if(project!=="all"&&`${f.studio}:${f.projectId}`!==project)return false;const q=search.trim().toLowerCase();return !q||`${f.title} ${f.projectName} ${f.assetTypeLabel}`.toLowerCase().includes(q);}),[families,studio,project,search,showSingleVersions]);
  ...
  return <main className="min-h-screen bg-[var(--background)] px-4 py-6 text-[var(--text-primary)] sm:px-7 lg:px-10">
    <div className="mx-auto max-w-[1500px]">
      ...
      <section className="mt-5 rounded-[1.6rem] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm"><div className="grid gap-3 lg:grid-cols-[1fr_190px_230px_190px]">
        ...
        <button type="button" onClick={()=>setShowSingleVersions(value=>!value)} className={`h-12 rounded-2xl border px-4 text-xs font-black transition ${showSingleVersions?"border-[var(--border)] bg-[var(--surface-strong)] text-[var(--text-secondary)]":"border-[var(--accent-border)] bg-[var(--accent-soft)] text-[var(--accent-strong)]"}`}>{showSingleVersions?"Show version histories":"Multiple versions only"}</button>
      </div></section>
      ...
      <section className="mt-5 overflow-hidden rounded-[1.6rem] border border-[var(--border)] bg-[var(--surface)] shadow-sm">
        <div className="hidden grid-cols-[80px_minmax(220px,1.2fr)_minmax(180px,.8fr)_120px_160px_28px] gap-4 border-b border-[var(--border)] bg-[var(--surface-strong)] px-4 py-3 text-[.56rem] font-black uppercase tracking-[.14em] text-[var(--text-muted)] md:grid"><span>Preview</span><span>Asset and project</span><span>Version trail</span><span>Status</span><span>Last updated</span><span/></div>
        <div className="divide-y divide-[var(--border)]">{visibleFamilies.map(f=><FamilyHistoryRow key={f.familyKey} family={f} onOpen={()=>{setActive(f);setCompare([]);setActionError("");}}/>)}</div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] bg-[var(--surface-strong)] px-4 py-3"><p className="text-xs font-bold text-[var(--text-muted)]">Showing {visibleFamilies.length} of {filtered.length} {showSingleVersions?"tracked assets":"version histories"}</p>{visibleFamilies.length<filtered.length&&<button type="button" onClick={()=>setVisibleCount(count=>count+30)} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-xs font-black transition hover:border-[var(--accent-border)] hover:text-[var(--accent-strong)]">Load 30 more</button>}</div>
      </section>}
    </div>
...
function FamilyHistoryRow({family,onOpen}:{family:Family;onOpen:()=>void}){const accent=STUDIO_ACCENTS[family.studio]||STUDIO_ACCENTS.other;const trail=family.versions.slice(0,4).reverse();return <button type="button" onClick={onOpen} className="group grid w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-[var(--accent-soft)] md:grid-cols-[80px_minmax(220px,1.2fr)_minmax(180px,.8fr)_120px_160px_28px] md:gap-4"><span className="grid h-14 w-20 place-items-center overflow-hidden rounded-xl bg-[var(--surface-hover)]">{family.previewUrl?<img src={family.previewUrl} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover"/>:<ImageIcon size={22} className="text-[var(--text-muted)]"/>}</span><span className="min-w-0"><span className="block text-[.55rem] font-black uppercase tracking-[.13em]" style={{color:accent}}>{family.studioLabel} · {family.assetTypeLabel}</span><strong className="mt-1 block truncate text-sm font-black">{family.title}</strong><span className="mt-0.5 block truncate text-xs font-semibold text-[var(--text-muted)]">{family.projectName}</span></span><span className="min-w-0"><span className="flex flex-wrap items-center gap-1.5">{family.versions.length>4&&<span className="text-[.58rem] font-bold text-[var(--text-muted)]">+{family.versions.length-4}</span>}{trail.map(version=><span key={version.id} className={`rounded-full px-2 py-1 text-[.52rem] font-black ${version.isCurrent?"bg-[var(--accent-strong)] text-white":"border border-[var(--border)] bg-[var(--surface-strong)] text-[var(--text-secondary)]"}`}>V{version.versionNumber}</span>)}</span><span className="mt-1.5 block text-[.6rem] font-bold text-[var(--text-muted)]">{family.versions.length} saved version{family.versions.length===1?"":"s"}</span></span><span><Status status={family.status}/></span><span className="text-xs font-bold text-[var(--text-muted)]">{date(family.updatedAt)}</span><ChevronRight size={17} className="hidden text-[var(--text-muted)] transition group-hover:translate-x-1 group-hover:text-[var(--accent-strong)] md:block"/></button>}
