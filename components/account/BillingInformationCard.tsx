...
export default function BillingInformationCard() {
  ...
  async function save() {
    ...
    try {
      ...
      setProfile({ ...empty, ...(result.profile || profile) });
      setSaved(true);
    } catch (value) {
      ...
      {loading ? <div className="grid min-h-32 place-items-center"><LoaderCircle className="animate-spin text-[var(--accent-strong)]"/></div> : (
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2 grid grid-cols-2 gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-1.5">
            <button type="button" onClick={() => setProfile({ ...profile, customer_type: "personal" })} className={`flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black transition ${profile.customer_type === "personal" ? "bg-[var(--surface-strong)] text-[var(--accent-strong)] shadow-sm" : "text-[var(--text-muted)]"}`}><UserRound size={15}/> Personal</button>
            <button type="button" onClick={() => setProfile({ ...profile, customer_type: "business" })} className={`flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black transition ${profile.customer_type === "business" ? "bg-[var(--surface-strong)] text-[var(--accent-strong)] shadow-sm" : "text-[var(--text-muted)]"}`}><Building2 size={15}/> Business</button>
          </div>
          ...
          <Field label="State / region" value={profile.state_region} onChange={(value) => setProfile({ ...profile, state_region: value })}/>
          <Field label="Postcode / ZIP *" value={profile.postal_code} onChange={(value) => setProfile({ ...profile, postal_code: value })}/>
          {error && <p className="sm:col-span-2 text-sm font-bold text-red-500">{error}</p>}
          <div className="sm:col-span-2"><Button type="button" onClick={save} disabled={saving}>{saving ? <LoaderCircle size={16} className="animate-spin"/> : <Save size={16}/>} Save billing information</Button></div>
        </div>
      )}
    </GlassCard>
  );
...
function Field({ label, value, onChange, type = "text", placeholder, maxLength, className = "" }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; maxLength?: number; className?: string }) {
  return <label className={className}><span className="mb-2 block text-[.62rem] font-black uppercase tracking-[.13em] text-[var(--text-muted)]">{label}</span><input className="heyy-input w-full" type={type} placeholder={placeholder} maxLength={maxLength} value={value || ""} onChange={(event) => onChange(event.target.value)}/></label>;
}
