...
type TopazChoice = { model: string; endpoint: string; generative: boolean };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!process.env.TOPAZ_API_KEY) return NextResponse.json({ error: "Enhancement service is not configured." }, { status: 503 });
    ...
    if (outputWidth * outputHeight > MAX_OUTPUT_PIXELS) {
      return NextResponse.json({
        error: `This ${scale}× upscale would exceed the safety limit of 100 megapixels. Use a smaller source${scale === 4 ? " or choose 2×" : ""}.`,
      }, { status: 400 });
    }
    ...
    form.append("image", new Blob([source], { type: mimeType }), String(body?.filename || "source-image").slice(0, 160));
    ...
    if (!response.ok) {
      console.error("Enhancement provider start error:", data?.error?.message || data?.message || response.status);
      throw new Error("The enhancement service could not start this image.");
    }

    const providerId = String(data?.process_id || response.headers.get("x-process-id") || data?.request_id || data?.id || "").trim();
    ...
    if (updateError || !updated) throw new Error(updateError?.message || "Enhancement job could not be saved.");

    return NextResponse.json({ success: true, jobId: startedJob.jobId, status: "processing", creditsReserved: startedJob.creditsReserved });
  } catch (error) {
    const reason = error instanceof Error ? error.message : "Upscale start failed";
    if (admin && startedJob) {
    ...
    return NextResponse.json({ error: "Enhancement could not start." }, { status: 500 });
  }
}

function getTopazChoice(value: string): TopazChoice {
  const key = value.toLowerCase();
  const standardEndpoint = process.env.TOPAZ_IMAGE_ENDPOINT || process.env.TOPAZ_ENHANCE_ENDPOINT || "https://api.topazlabs.com/image/v1/enhance/async";
  const generativeEndpoint = process.env.TOPAZ_ENHANCE_GENERATIVE_ENDPOINT || "https://api.topazlabs.com/image/v1/enhance-gen/async";
  if (key.includes("strong") || key.includes("recover")) return { model: process.env.TOPAZ_MODEL_STRONG || "Recover 3", endpoint: generativeEndpoint, generative: true };
  if (key.includes("high")) return { model: process.env.TOPAZ_MODEL_HIGH_FIDELITY || "High Fidelity V2", endpoint: standardEndpoint, generative: false };
  if (key.includes("low")) return { model: process.env.TOPAZ_MODEL_LOW_RES || "Low Resolution V2", endpoint: standardEndpoint, generative: false };
  if (key.includes("art") || key.includes("illustration") || key.includes("cgi")) return { model: process.env.TOPAZ_MODEL_ART || "CGI", endpoint: standardEndpoint, generative: false };
  if (key.includes("text") || key.includes("shape")) return { model: process.env.TOPAZ_MODEL_TEXT || "Text Refine", endpoint: standardEndpoint, generative: false };
  return { model: process.env.TOPAZ_MODEL_STANDARD || "Standard V2", endpoint: standardEndpoint, generative: false };
}

function sourceFingerprint(buffer: Buffer) {
  // A small deterministic fingerprint is enough for the active-job dedupe key;
  // source bytes are never persisted in generation_jobs.
  let hash = 2166136261;
  ...
  return `${buffer.length}:${(hash >>> 0).toString(16)}`;
}
