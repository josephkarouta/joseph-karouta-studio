...
async function hydrateContactRows(client: AdminClient, rows: UnknownRow[]) {
  return Promise.all(rows.map(async (row) => {
    ...
    return {
      ...
      contact_project_id: firstText(metadata.project_id),
      contact_project_name: firstText(metadata.project_name),
      contact_studio: firstText(metadata.studio),
      contact_service_id: firstText(metadata.service_id),
      contact_source_job_id: firstText(metadata.source_job_id),
      contact_production_request_id: firstText(metadata.converted_studio_request_id),
      contact_production_project_id: firstText(metadata.converted_project_id),
      contact_production_only: metadata.production_only === true,
