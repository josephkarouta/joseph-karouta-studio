  ...
  try {
    ...
    if (!customer) {
      return NextResponse.json(
        {
          success: false,
          error: "No paid billing account exists for this user yet.",
        },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof ApiAuthError ? error.message : "Unable to open billing management. Please try again.",
      },
      { status },
    );
  }
