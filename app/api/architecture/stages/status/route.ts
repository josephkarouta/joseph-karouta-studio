...
export async function GET(request: Request) {
  try {
    ...
    if (!job) return NextResponse.json({ error: "Architecture generation job not found." }, { status: 404 });

    if (job.status === "failed" || job.status === "cancelled") {
      return NextResponse.json({
        success: true,
        status: "failed",
        error: job.error || "Architecture content generation failed. Your credits were returned.",
      });
    }
    ...
    if (!projectId) throw new Error("Architecture generation job has no project ID.");
    ...
    return NextResponse.json({
      ...
      status: "succeeded",
      mode: "live",
      plan: job.output?.plan || job.input?.planName || null,
    ...
    console.error("Architecture stage status error:", error);
    return NextResponse.json(
