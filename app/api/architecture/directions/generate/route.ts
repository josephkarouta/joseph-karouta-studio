      ...
      cookies: {
        ...
        setAll(
          ...
          try {
            ...
            // Cookie writes are not required after the response is committed.
          }
        },
      },
    },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [projectResult, siteResult, planningResult, existingResult, materialsResult] = await Promise.all([
      ...
      supabase.from("architecture_directions").select("id,direction_number,is_selected,generation_json,image_storage_path").eq("project_id", projectId).eq("user_id", user.id),
    ...
    if (selectedMaterials.length < minimumMaterials) {
      return NextResponse.json(
        { success: false, error: `Select at least ${minimumMaterials} material${minimumMaterials === 1 ? "" : "s"} before generating directions.` },
        { status: 400 },
      );
    }
    ...
    const aiMode = process.env.NEXT_PUBLIC_MOCK_IMAGES === "true" ? "demo" : getAiMode();
    ...
    if (aiMode !== "demo") {
      ...
      const reservation = await reserveCredits({
        ...
        metadata: { project_id: projectId, studio: "architecture_studio", tool: "directions", direction_number: directionNumber || "all" },
      });
    ...
    if (upsertError) {
      const generatedPaths = payloads.flatMap((payload) => {
        const json = payload.generation_json && typeof payload.generation_json === "object"
          ? payload.generation_json as Record<string, unknown>
          : {};
        return [
          payload.image_storage_path,
          ...assetPaths(json.preview_assets),
          ...assetPaths(json.final_assets),
        ].filter((path): path is string => typeof path === "string" && Boolean(path));
      });
      if (generatedPaths.length) {
        await supabase.storage.from("architecture-files").remove(Array.from(new Set(generatedPaths)));
      }
    ...
    if (oldStoragePaths.length) {
      await supabase.storage.from("architecture-files").remove(oldStoragePaths);
    }
    ...
    if (creditReservationId && creditAdmin) {
      await commitCredits(creditAdmin, creditReservationId, { project_id: projectId, directions: numbers.length });
    ...
    return NextResponse.json({
      ...
      partialImageFailure: false,
    });
  } catch (error) {
    if (creditReservationId && creditAdmin) await refundCredits(creditAdmin, creditReservationId, error instanceof Error ? error.message : "Architecture direction generation failed");
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Architecture Directions could not be prepared." },
      { status: 500 },
    );
  }
}
