...
import { getAiMode, getAiPlanConfig, resolveAiPlan, type ImageGenerationTier } from "@/lib/ai/config";
...
export async function POST(request: Request) {
  try {
    ...
    async function paidGenerate<T>(
      ...
      const paid = await withCreditReservation({
        admin,
        userId: authenticatedUserId,
        action,
        metadata: { project_id: projectId, studio: "architecture_studio", ...metadata },
        work: async () => work(),
      });
      return paid.result;
    }
    ...
    if (targetType === "direction") {
      ...
      return NextResponse.json({ success: true, mode, targetType, quality, direction: updated });
    }
    ...
    if (targetType === "concept") {
      ...
      return NextResponse.json({ success: true, mode, targetType, quality, concept: updated });
    }
    ...
    if (mode === "demo") {
      ...
      if (sourceGeometryLocked) {
        return NextResponse.json(
          {
            success: false,
            error: "Existing Design source plans are preserved from the uploaded drawings and are not regenerated with AI. Use the Source Plan Organizer in the Plans section.",
          },
          { status: 409 },
        );
      }
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Architecture image could not be generated.",
      },
      { status: 500 },
    );
  }
