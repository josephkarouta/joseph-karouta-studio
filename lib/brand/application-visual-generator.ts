...
async function storeApplicationOutput(
...
export async function generateBrandApplicationVisual(
  storageContext: BrandImageStorageContext,
  body: any,
) {
  ...
  if (!project?.id || !applicationId) {
    throw new Error("A Brand project and application are required.");
  }
  if (!logoReferenceUrl) {
    throw new Error("Select or upload the project logo before generating this application.");
  }

  const createdPaths: string[] = [];
  const storeTrackedOutput = async (
    args: Parameters<typeof storeApplicationOutput>[1],
  ) => {
    const stored = await storeApplicationOutput(storageContext, args);
    createdPaths.push(stored.storagePath);
    return stored;
  };
...
const openai = imageProvider === "openai" ? getBackgroundOpenAI() : null;
...
- Produce a fresh composition for this regeneration while preserving the approved identity. Internal variation reference: ${variationId}. Never print this reference.`,
  ...
  const generated: Array<{ stored: GeneratedOutput; usage: unknown }> = [];
...
- Produce a fresh composition for this regeneration while preserving the approved identity. Internal variation reference: ${variationId}. Never print this reference.`,
      ...
      const stored = await storeTrackedOutput({
        buffer: art.buffer,
        applicationId,
      ...
      generated.push({ stored, usage: art.usage });
...
return {
  ...
  imageModel:
    ...
    if (createdPaths.length) {
      const { error: cleanupError } = await storageContext.admin.storage
        .from("project-assets")
        .remove(createdPaths);
      if (cleanupError) {
        console.error("Brand application partial-output cleanup failed:", cleanupError.message);
      }
    }
    throw error;
  }
