...
export async function accountExportSnapshot(admin: SupabaseClient, userId: string) {
  const tables = [
    "account_preferences",
    "user_subscriptions",
    "credit_wallets",
  ...
  for (const table of tables) {
    ...
    if (error) {
      ...
      // A single legacy table should not prevent the user from exporting the
      // rest of their account data.
      console.warn(`Account export skipped ${table}:`, error.message);
      results[table] = [];
      continue;
    }
    results[table] = data || [];
